...
import CarouselBullets from './CarouselBullets.vue'
...
export default {
  ...
  components: { CarouselBullets, CarouselControls },
  props: {
    type: {
      type: String,
      default: 'slider'
  ...
  setup() {
    ...
    const slidesCount = computed(() => slots.default({})[0]?.children?.length)

    return { is2xl, slots, slidesCount }
  },
  data() {
    return {
      glide: null
  ...
  methods: {
     ...
     */
    goToSlideByClick() {
      this.glide.on('glide:slide-click', e => this.go(`=${e}`))
