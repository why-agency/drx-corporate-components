<template>
  <div>
    ...
    >
      <o-09-01-JobMarketFilterbarMobileDetail v-if="jobsStore.activeFilterView" />
    </transition>
    <!-- START filter detail view -->

        <!-- START active filter bar -->
    <transition
      enter-class="opacity-0 transform translate-y-80"
      enter-active-class="transition ease-out duration-300"
      enter-to-class="opacity-100 transform translate-y-0"
      leave-to-class="transform translate-y-full"
      leave-active-class="transition ease-in duration-300"
    >
      <o-09-01-JobMarketFilterbarMobileList v-if="jobsStore.isFilterBarActive" />
    </transition>
    <!-- END active filter bar -->

    <button
      v-if="!jobsStore.isFilterBarActive"
      class="fixed flex w-full px-8 -mx-6 bottom-10 z-60 drop-shadow-2xl"
      @click="showFilterBar"
    >
      <p class="flex items-center flex-1 px-4 h-12 bg-primary text-sand text-button">
        Jetzt Filter auswählen
      </p>
      <div class="flex items-center justify-center w-12 h-12 bg-sand text-primary">
        <IconFilter />
      </div>
    </button>
  </div>
